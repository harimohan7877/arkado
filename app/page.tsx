...
export default function HomePage() {
  ...
  const featuredTitle = sections?.featured_bundles?.title || settings?.homepage?.featured_section_title || "Featured Bundles";
  const hotDealsTitle = sections?.hot_deals?.title || settings?.homepage?.hot_deals_title || "Today's Hot Deals";
  const newArrivalsTitle = sections?.new_arrivals?.title || settings?.homepage?.new_arrivals_title || "New Arrivals";
  ...
  const orderedSections = getOrderedSectionKeys(settings);

  const renderSection = (sectionKey: SectionKey) => {
    const visibility = getSectionVisibility(settings, sectionKey);
      ...
      case "featured_bundles":
        content = (
          <section id="courses" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <FeaturedGrid
              title={featuredTitle}
              courses={featuredDeals.length > 0 ? featuredDeals : latestProducts}
              onBuyNow={handleBuyNow}
              onOpenSample={handleOpenSample}
              viewAllHref="/exams"
            />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-w-4xl mx-auto">
                {(settings?.homepage?.faqs && settings.homepage.faqs.length > 0
                  ...
                  : [
                      ...
                      {
                        question: "Need help?",
                        answer: `WhatsApp ${settings?.contact?.whatsapp_number || DEFAULT_SETTINGS.whatsapp_support_number} — our team responds quickly.`,
                      },
                    ]
                ).map((f) => (
                  <div
    ...
    if (visibility.className) {
      return (
        <div key={sectionKey} className={visibility.className}>
          {content}
        </div>
      );
    }
